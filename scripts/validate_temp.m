...
% Handles the output from RIFFA when it is already 64 bit floating point
function validate_temp(steps, runs, blocks, chains)
    ...
    lpr_sum = 0.0;
    temp_mean = 0.0;
    lpr_in = fscanf(file, '%f');
    fclose(file);
    x_in = fscanf(file_x, '%f');
